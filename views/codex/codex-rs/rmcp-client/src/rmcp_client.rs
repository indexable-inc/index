...
/// Interface for forwarding a server's custom (non-spec) notifications to the
/// owner of this client. See [`RmcpClient::initialize_with_notifications`].
...
impl RmcpClient {
    ...
    ///
    /// Custom notifications from the server are dropped; use
    /// [`Self::initialize_with_notifications`] to receive them.
    pub async fn initialize(
        ...
        self.initialize_with_notifications(params, timeout, send_elicitation, None)
            .await
    }

    /// [`Self::initialize`], additionally routing every custom notification the
    /// server sends (any `notifications/*` method outside the MCP spec) to
    /// `send_notification` for the lifetime of the connection.
    #[instrument(level = "trace", skip_all)]
    pub async fn initialize_with_notifications(
        &self,
        params: InitializeRequestParams,
        timeout: Option<Duration>,
        send_elicitation: SendElicitation,
