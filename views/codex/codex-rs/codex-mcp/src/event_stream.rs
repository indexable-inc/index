...
pub(crate) struct EventStreamConnectionSettings {
    ...
    /// Channel notifications are connection-local, so event-stream clients
    /// route them through the same slot as the task's primary clients.
    ...
    pub async fn open(
        ...
        tokio::select! {
            ...
            result = async {
                ...
                client.initialize_with_notifications(
                    ...
                    Some(connection.notifications.sender(CODEX_APPS_MCP_SERVER_NAME.to_string())),
                ).await?;
                McpEventStream::open(
                    client,
