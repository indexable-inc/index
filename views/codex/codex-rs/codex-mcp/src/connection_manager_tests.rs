...
async fn reconcile_reusable_server_with_mcp_config(
    ...
    reconcile_reusable_server_with_notification_handler(
        previous,
        config,
        runtime_context,
        mcp_config,
...
async fn reconcile_reusable_server_with_notification_handler(
    previous: &McpConnectionSet,
    config: McpServerConfig,
    runtime_context: McpRuntimeContext,
    mcp_config: crate::McpConfig,
...
#[tokio::test]
async fn reconciliation_installs_notification_handler_for_senders_of_reused_servers() {
    let runtime_context = reusable_server_runtime_context();
    let config = reusable_server_config("http://127.0.0.1:1");
    let previous = manager_with_reusable_ready_server(
        &config,
    ...
    .await;
    // The sink a client connected under the previous set keeps holding.
    let send = previous
        .elicitation_requests
        .notifications
        .sender("docs".to_string());
    let handler = Arc::new(RecordingNotificationHandler::default());
    let codex_home = tempdir().expect("tempdir");

    let reconciled = reconcile_reusable_server_with_notification_handler(
        &previous,
        config,
        runtime_context,
        crate::mcp::tests::test_mcp_config(codex_home.path().to_path_buf()),
    ...
    send(CustomNotification::new(
        ...
        Some(serde_json::json!({"content": "reused connection"})),
    ))
    .await
    .expect("routed through the reused set's slot");
    assert_eq!(
        handler
            .recorded()
            .iter()
