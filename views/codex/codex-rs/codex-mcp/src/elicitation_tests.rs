...
use crate::notification::McpChannelNotification;
use crate::notification::McpNotificationHandler;
use crate::notification::McpNotificationHandlerHandle;
use crate::notification::NotificationRouter;
use crate::notification::testing::RecordingNotificationHandler;
...
fn notification_router(handler: Option<McpNotificationHandlerHandle>) -> NotificationRouter {
    let router = NotificationRouter::default();
    router.set_handler(handler);
    router
}

fn channel_notification(method: &str, params: Value) -> CustomNotification {
...
async fn channel_notification_sender_routes_claude_channel_events() {
    ...
    let router = notification_router(Some(handler.clone()));

    let send = router.sender("ix".to_string());
    send(channel_notification(
...
#[tokio::test]
async fn channel_notification_sender_ignores_other_methods() {
    let handler = Arc::new(RecordingNotificationHandler::default());
    let router = notification_router(Some(handler.clone()));
    let send = router.sender("ix".to_string());
...
#[tokio::test]
async fn channel_notification_sender_rejects_malformed_events() {
    let handler = Arc::new(RecordingNotificationHandler::default());
    let router = notification_router(Some(handler.clone()));
    let send = router.sender("ix".to_string());
    ...
    for notification in [
        channel_notification(
            ...
            json!({"meta": {"no": "content"}}),
        ),
        channel_notification("notifications/codex/channel", json!({"content": 42})),
        channel_notification(
            "notifications/codex/channel",
            json!({"content": "x", "meta": "not an object"}),
        ),
        channel_notification(
            "notifications/codex/channel",
            json!({"content": "x", "meta": null}),
        ),
        channel_notification("notifications/codex/channel", json!("not an object")),
        CustomNotification::new("notifications/codex/channel", None),
    ] {
        let params = notification.params.clone();
        assert!(
            send(notification).await.is_err(),
            "accepted malformed params {params:?}"
        );
    }
...
#[tokio::test]
async fn channel_notification_sender_propagates_handler_errors() {
    struct FailingHandler;
    impl McpNotificationHandler for FailingHandler {
        fn handle(&self, _: McpChannelNotification) -> BoxFuture<'static, Result<()>> {
            Box::pin(async { Err(anyhow::anyhow!("turn not submitted")) })
        }
    }
    let router = notification_router(Some(Arc::new(FailingHandler)));
    let send = router.sender("ix".to_string());

    let error = send(channel_notification(
        "notifications/codex/channel",
        json!({"content": "x"}),
    ))
    .await
    ...
    assert_eq!(error.to_string(), "turn not submitted");
}

#[tokio::test]
async fn channel_notification_handler_installed_later_reaches_existing_senders() {
    let router = notification_router(None);
    let send = router.sender("ix".to_string());

    // No handler yet: dropped, not an error.
    send(channel_notification(
        "notifications/codex/channel",
        json!({"content": "before"}),
    ))
    ...
    // A reconciliation that reuses the connection set installs the handler on
    // the shared slot; senders made before it must pick it up.
    let handler = Arc::new(RecordingNotificationHandler::default());
    router.set_handler(Some(handler.clone()));
    send(channel_notification(
        "notifications/codex/channel",
        json!({"content": "during"}),
    ))
    ...
    // Clearing it stops delivery through the same sender.
