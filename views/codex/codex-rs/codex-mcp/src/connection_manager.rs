...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        // A reused set keeps its connected clients; their senders resolve the
        // handler through this shared slot, so installing it here reaches them.
        elicitation_requests
            .notifications
            .set_handler(notification_handler);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            if is_host_owned_codex_apps {
                event_stream_connection = Some(Arc::new(EventStreamConnectionSettings {
                    ...
                    notifications: elicitation_requests.notifications.clone(),
                }));
            }
