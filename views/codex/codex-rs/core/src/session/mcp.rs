...
/// Turns MCP channel notifications (`notifications/claude/channel`,
/// `notifications/codex/channel`) into input on this thread: the content is
/// wrapped as a `<channel ...>` message and submitted like typed text
/// (`TurnInputMode::StartOrSteer`: steers the active turn, else starts one).
/// Holds a weak handle on the session's submission loop rather than the
/// session itself: it can be built from `&Session` wherever the MCP runtime
/// input is assembled, and it never keeps the loop alive (the loop exits once
/// every strong sender is dropped). A notification after that point fails.
struct ChannelMcpNotificationHandler {
    tx_sub: async_channel::WeakSender<Submission>,
}
...
impl McpNotificationHandler for ChannelMcpNotificationHandler {
    fn handle(
        ...
        Box::pin(async move {
            let Some(tx_sub) = tx_sub.upgrade() else {
                anyhow::bail!("session submission loop has exited");
            };
            ...
            tx_sub
                ...
                .await
                .map_err(|_| anyhow::anyhow!("session submission loop has exited"))?;
            let submission = reply_rx
                .await
                .map_err(|_| anyhow::anyhow!("session dropped the channel turn input"))??;
            match submission {
                TurnInputSubmission::Started { .. } | TurnInputSubmission::Steered { .. } => Ok(()),
                TurnInputSubmission::NotSubmitted { reason } => Err(anyhow::anyhow!(
                    "channel notification from {server_name} was not submitted: {reason:?}"
                )),
            }
        })
    }
...
/// `<channel source="<server>" [method="..."] [meta-key="value" ...]>content</channel>`.
/// `method` is omitted for the Claude spelling (the common case). Meta keys
/// become attributes: they must be XML names (`[A-Za-z_][A-Za-z0-9_.-]*`) and
/// may not shadow `source` or `method` (a server could otherwise forge its
/// attribution); anything else rejects the whole notification. Keys are sorted
/// so the rendering is deterministic; string values are used as-is, others are
/// JSON-encoded. Text and attribute values are XML-escaped.
fn format_mcp_channel_notification(notification: McpChannelNotification) -> anyhow::Result<String> {
    let source = xml_escape_attribute(&notification.server_name);
    let mut attributes = vec![format!("source=\"{source}\"")];
    if notification.method != "notifications/claude/channel" {
        let method = xml_escape_attribute(&notification.method);
        attributes.push(format!("method=\"{method}\""));
    }
    if let Some(meta) = notification.meta {
        ...
        for (key, value) in meta {
            validate_channel_attribute_name(&key)?;
            let value = match value {
                serde_json::Value::String(value) => value,
                value => serde_json::to_string(&value)?,
            };
            let value = xml_escape_attribute(&value);
            attributes.push(format!("{key}=\"{value}\""));
        }
    }
    let attributes = attributes.join(" ");
    let content = xml_escape_text(&notification.content);
    Ok(format!("<channel {attributes}>{content}</channel>"))
}

fn validate_channel_attribute_name(key: &str) -> anyhow::Result<()> {
    if matches!(key, "source" | "method") {
        anyhow::bail!("channel notification meta key `{key}` is reserved");
    }
    ...
    if !(valid_start && valid_rest) {
        anyhow::bail!("channel notification meta key `{key}` is not a valid attribute name");
    }
    Ok(())
}
...
impl Session {
    ...
    pub(crate) fn mcp_notification_handler(&self) -> McpNotificationHandlerHandle {
        Arc::new(ChannelMcpNotificationHandler {
