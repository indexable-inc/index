...
pub(crate) struct Session {
    ...
    /// Weak handle on the submission loop, for session-owned callbacks (MCP
    /// channel notifications) that queue ordinary submissions. Weak so the
    /// session never keeps its own loop alive: the loop exits once every
    /// strong sender (`SessionIo`) is dropped.
    pub(super) tx_sub: async_channel::WeakSender<Submission>,
