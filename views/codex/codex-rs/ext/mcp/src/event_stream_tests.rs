...
impl Fixture {
    pub async fn new() -> Result<Self> {
        Self::with_notification_handler(/*notification_handler*/ None).await
    }

    pub async fn with_notification_handler(
...
/// Forwards every channel notification to the test over a channel.
struct ForwardingChannelHandler(mpsc::UnboundedSender<McpChannelNotification>);

impl McpNotificationHandler for ForwardingChannelHandler {
...
#[tokio::test]
async fn event_connections_route_channel_notifications_to_the_runtime_handler() -> Result<()> {
    let (handled_tx, mut handled) = mpsc::unbounded_channel();
    let mut fixture =
        Fixture::with_notification_handler(Some(Arc::new(ForwardingChannelHandler(handled_tx))))
            .await?;
    ...
    // A channel notification on the event connection is not an event: it must
    // reach the runtime's handler, attributed to the Codex Apps server.
    remote.notifications.send(json!({
        "jsonrpc": "2.0",
        "method": "notifications/codex/channel",
        "params": {"content": "deploy finished", "meta": {"severity": "info"}},
    }))?;
    assert_eq!(
        timeout(WAIT, handled.recv()).await?,
        Some(McpChannelNotification {
            server_name: CODEX_APPS_MCP_SERVER_NAME.to_string(),
            method: "notifications/codex/channel".to_string(),
            content: "deploy finished".to_string(),
            meta: json!({"severity": "info"}).as_object().cloned(),
        })
    );

    // The event stream itself still receives its events.
